...
def find_optimal_parameters_sa(problem, n, name):
    ...
    for r in decay:
        ...
        best_state, best_fitness, fitness_curve = mlrose.simulated_annealing( problem,schedule=schedule, max_attempts=1000, max_iters=100000,init_state=init_state, curve=True) 
        fitness_value.append( best_fitness)
        fitness_curve_arr.append(fitness_curve)
    fitness_value=np.array( fitness_value)
    print( fitness_value)
    ...
    plt.figure()
    plt.grid()
    plt.plot(fitness_curve_arr[0], label =' pop 200: mutation_pr')
    plt.plot(fitness_curve_arr[1], label=' pop 500: mutation_prob:0.001')
    plt.plot(fitness_curve_arr[2], label=' pop 200: mutation_prob:0.01')
    plt.plot(fitness_curve_arr[3], label=' pop 500: mutation_prob:0.01')
    plt.plot(fitness_curve_arr[4], label=' pop 500: mutation_prob:0.01')
    plt.xlabel( 'iterations')
    plt.ylabel('fitness value ')
    plt.title('variation of fitness with Mutation and population size')
    plt.show()
...
def sa_different_schedule(problem, name, n):
	...
	for s in schedule:
		best_state, best_fitness, fitness_curve = mlrose.simulated_annealing(problem, schedule = s, 
			max_attempts = 10000, max_iters=100000, init_state = init_state, curve=True)
		fitness_curve_arr.append( fitness_curve)
		fitness_values.append( best_fitness)
...
def compare_algorithms_iterations( problem, ga_param, sa_param, rhc_param, mimic_param, name, n):
	...
	best_state, best_fitness_mimic, fitness_curve_mimic = mlrose.mimic(problem, pop_size =mimic_param[0], keep_pct = mimic_param[1],max_attempts =100, max_iters=100, curve=True,fast_mimic=True)
	et = time.time()
	mimic_time = et-st
...
def four_peaks_compare_algorithms(problem ,ga_param, sa_param, rhc_param, mimic_param, name):
	...
	for n in range(5,100,20):
		...
		st = time.time()
		best_state_mimic, best_fitness_mimic, fitness_curve_mimic = mlrose.mimic(problem,pop_size=mimic_param[0], max_attempts = 500, 
			max_iters=500,keep_pct=mimic_param[1], curve=True, fast_mimic=True)
		end = time.time()
		mimic_time = end-st
		print(mimic_time,n)
...
def compare_algorithms_func_eval( problem, ga_param, sa_param, rhc_param, mimic_param, name):
	...
	for n in range( 40 , 101, 20):
		...
		eval_count=0
		best_state, best_fitness= mlrose.genetic_alg( problem, pop_size=ga_param[0], mutation_prob=ga_param[1], max_attempts=1000,max_iters =1000 )
		func_eval_ga.append( eval_count)
		print(eval_count)

		eval_count = 0
		best_state, best_fitness= mlrose.simulated_annealing( problem, schedule=schedule, max_attempts=1000 ,max_iters =1000 , init_state=init_state)
	...
	plt.figure()
	plt.grid()
	plt.plot(np.arange(40,101,20),func_eval_sa,label='SA')
	plt.plot(np.arange(40,101,20),func_eval_rhc,label = 'RHC')
	plt.plot(np.arange(40,101,20),func_eval_ga, label = 'GA')
	plt.plot(np.arange(40,101,20),func_eval_mimic, label = 'MIMIC')
...
def Four_peaks():
	...
	#find_optimal_parameters_ga(problem, n,'4peaks')
	#optimal_parameters_rhc(problem, n, '4peaks')
	#find_optimal_parameters_mimic( problem,n,'4peaks' )
	#sa_different_schedule( problem, '4peaks', n)
	
	#find_optimal_parameters_sa( problem, n, '4peaks')
	#compare_algorithms_tpct( ga_param, sa_param, rhc_param, mimic_param, '4peaks')
	breakpoint()
	#compare_algorithms_iterations(problem, ga_param, sa_param, rhc_param, mimic_param,'4peaks',n)
	four_peaks_compare_algorithms( problem, ga_param, sa_param, rhc_param, mimic_param,'4peaks')
	#compare_algorithms_func_eval(problem, ga_param, sa_param, rhc_param, mimic_param,'4peaks')
